...
public class ToDoListManager {
    ...
    public User addUser(String name, String email) {
        User user = new User(UUID.randomUUID().toString().substring(0, 5), name, email);
        users.add(user);
        return user;
    }
    ...
    public Task addTask(String title, String description, String category, String assignedUserId) {
        Task task = new Task(UUID.randomUUID().toString().substring(0, 5), title, description, category, assignedUserId, "pending");
        tasks.add(task);
        return task;
    }
